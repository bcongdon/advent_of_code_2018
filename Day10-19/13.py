...
def simulate(rail_map, carts):
    ...
    while True:
        ...
        for cart_loc in sorted_locs:
            ...
            dx, dy = 0, 0
            if map_tile == '\\':
                direction = left_turns[direction] if direction in '^v' else right_turns[direction]
            elif map_tile == '/':
                direction = right_turns[direction] if direction in '^v' else left_turns[direction]
            elif map_tile == '+':
                if state == 0:
