...
def calculate(lines):
    ...
    max_guard_id, guard_schedule = max(guards.items(), key=lambda kv: kv[1].sum())
    part1 = max_guard_id * guard_schedule.argmax()

    max_minutes_asleep = max(minutes.max() for guard_id, minutes in guards.items())
    max_guard_id, minute_asleep = next(
        (guard_id, minutes.argmax())
        for guard_id, minutes in guards.items()
        if minutes.max() == max_minutes_asleep
    )
